...
import type { QueueFilters, QueueFormValues, QueueRecord, QueueStatus } from "@/types/queue";
import type { AuthUser } from "@/types/auth";
import type { RecordSource, UnifiedItemKind } from "@/types/unified";

function buildQueueStorageKeys(slug: string) {
  const safe = slug.trim().toLowerCase() || "company";
  return {
    LOCAL_STORAGE_KEY: `alx-entregas-fila-${safe}`,
    ANALYST_STORAGE_KEY: `alx-entregas-analista-${safe}`,
    AUTH_STORAGE_KEY: `alx-auth-session-${safe}`,
  };
}
...
function normalizeOrigem(origem: QueueRecord["origem"] | string | null | undefined): RecordSource {
...
function normalizeQueue(records: QueueRecord[]): QueueRecord[] {
  return ([...records]
    .map((record) => ({
      ...record,
...
export function createCompanyQueueStore(companyId: string, slug: string) {
  const keys = buildQueueStorageKeys(slug);

  function readAuthUser(): AuthUser | null {
    if (typeof window === "undefined") return null;
    const raw = window.localStorage.getItem(keys.AUTH_STORAGE_KEY);
    if (!raw) return null;
    try {
  ...
  function readLocalAnalyst() {
    ...
    return window.localStorage.getItem(keys.ANALYST_STORAGE_KEY) ?? "";
  }

  function writeLocalAnalyst(value: string) {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(keys.ANALYST_STORAGE_KEY, value);
    }
  }
  ...
  function readLocalQueue() {
    ...
    const raw = window.localStorage.getItem(keys.LOCAL_STORAGE_KEY);
  ...
  function writeLocalQueue(records: QueueRecord[]) {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(keys.LOCAL_STORAGE_KEY, JSON.stringify(records));
    }
  }
  ...
  return create<QueueStore>((set, get) => ({
    ...
    createRecord: async (values) => {
      ...
      const origem: RecordSource = payload.origem ?? "operacional";
      const tipo: UnifiedItemKind =
        payload.tipo ??
        (origem === "entregador"
          ? "ENTREGADOR"
          : tipo_atribuicao === "TPR"
      ...
      const analista = origem === "entregador" ? null : currentAnalyst ?? null;
      ...
      const nextQueue = normalizeQueue([...get().queue, nextRecord]);
    ...
    removeRecord: async (id) => {
      ...
      if (supabase) {
        const current = get().queue.find((record) => record.id === id);
        ...
        const { error } = await supabase
          .from("fila_registros")
          .update({ status, analista: analystUpdate } as never)
          .eq("id", id)
          .eq("company_id", companyId);
      ...
      const updatedRecords = get().queue.map((record) =>
        record.id === id
          ? { ...record, status, analista: analystUpdate }
          : record,
      );
      const nextQueue = normalizeQueue(updatedRecords) as QueueRecord[];
      writeLocalQueue(nextQueue);
      set({ queue: nextQueue, syncing: false });
    },
